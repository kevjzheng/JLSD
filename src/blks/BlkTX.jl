...
function drv_add_jitter!(drv, Vi_fir)
    ...
    @unpack prev_nui, V_prev_nui, Vext, tt_Vext = drv
    ...
    Vext[lastindex(V_prev_nui)+1:end] .= Vi_fir

    Vi_fir .= linear_interpolation(tt_Vext, Vext).(tvec)
end


function drv_jitter_tvec(;blk_size, osr, dcd, rj_osr, last_rj_osr, sj_amp_osr, sj_freq_osr, last_sj_phi)

    t_ui::Vector{Float64} = LinRange(0,blk_size*osr,blk_size+1)
    t_rj::Vector{Float64} = [last_rj_osr; rj_osr*randn(blk_size)]
    ...
    tvec_jitter = zeros(blk_size*osr)
